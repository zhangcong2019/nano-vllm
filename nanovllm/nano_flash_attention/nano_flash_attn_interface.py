...
from nanovllm.nano_flash_attention.nano_flash_attn_triton import flash_attn_2 as flash_attn_gpu
...
def _get_block_size_n(device, head_dim, is_dropout, is_causal):
    # This should match the block sizes in the xpu kernel
    assert head_dim <= 256
    major, minor = torch.xpu.get_device_capability(device)
...
@_torch_custom_op_wrapper("flash_attn::_flash_attn_forward", mutates_args=(), device_types="xpu")
...
def _flash_attn_forward_fake(
    ...
    if return_softmax:
        if torch.xpu.is_available() and torch.version.hip:
...
@_torch_custom_op_wrapper("flash_attn::_flash_attn_varlen_forward", mutates_args=(), device_types="xpu")
...
if torch.__version__ >= "2.4.0":
    print("Using torch.ops.flash_attn._flash_attn_varlen_forward for flash attention forward")
    _wrapped_flash_attn_varlen_forward = torch.ops.flash_attn._flash_attn_varlen_forward
else:
    print("Using custom _flash_attn_varlen_forward for flash attention forward")
    _wrapped_flash_attn_varlen_forward = _flash_attn_varlen_forward


@_torch_custom_op_wrapper("flash_attn::_flash_attn_backward", mutates_args=("dq", "dk", "dv"), device_types="xpu")
...
@_torch_custom_op_wrapper("flash_attn::_flash_attn_varlen_backward", mutates_args=("dq", "dk", "dv"), device_types="xpu")
