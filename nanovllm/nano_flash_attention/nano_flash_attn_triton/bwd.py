    ...
    if not autotune:
        ...
        if arch.name == "gfx942":
            if arch.cu_count < 304:
                preprocess_configs = [
                    triton.Config(
                        {"PRE_BLOCK": 64}, num_stages=1, num_warps=8
                    ),
                    triton.Config(
                        {"PRE_BLOCK": 64}, num_stages=2, num_warps=8
                    ),
                    triton.Config(
                        {"PRE_BLOCK": 128}, num_stages=1, num_warps=4
                    ),
                ]
                noncausal_configs = [
                    triton.Config(
                ...
                preprocess_configs = [
                    ...
                    triton.Config(
                        {"PRE_BLOCK": 64}, num_stages=1, num_warps=4
                    ),
                ]
                noncausal_configs = [
                    triton.Config(
            ...
            preprocess_configs = [
                ...
                triton.Config(
                    {"PRE_BLOCK": 64}, num_stages=2, num_warps=4
                ),
            ]
            noncausal_configs = [
                triton.Config(
...
def attention_backward_triton_impl(
    *,
    ...
    assert (
        q.is_xpu and q.device.index == current_device
    ), f"Device mismatch: Kernel will launch on xpu:{current_device}, but tensors are on {q.device}"
